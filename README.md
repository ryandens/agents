...
- **Credentials** — the cluster's password is generated by Terraform, never checked in,
  and stored as a SecureString in Parameter Store at `/agents/database-url` as a complete
  DSN. The instance role can read that one parameter (`infrastructure/ec2.tf`), and
  `agents.service` fetches it at start and writes it into a `0600` env file, the same way
  it handles the Anthropic key and the OAuth secret. The password never appears in the
  systemd unit, in user data, or in `docker inspect`.
